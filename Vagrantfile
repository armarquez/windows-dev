...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", path: "scripts/install-incus.sh", privileged: true
  config.vm.provision "shell", path: "scripts/configure-incus.sh", privileged: false
  
  # Copy SSH keys for easier access
  config.vm.provision "file", source: "~/.ssh/id_rsa.pub", destination: "/tmp/host_ssh_key.pub"
